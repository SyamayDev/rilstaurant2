...
<div class="container mt-4">
    <h1 class="mb-4">Edit Promo: <?= htmlspecialchars($promo->nama_promo) ?></h1>
    ...
    <?= form_open_multipart('admin_promo/edit/' . $promo->id_promo); ?>
        <div class="mb-3">
            <label for="nama_promo" class="form-label">Nama Promo</label>
            <input type="text" class="form-control" id="nama_promo" name="nama_promo" value="<?= set_value('nama_promo', $promo->nama_promo) ?>" required>
        </div>

        <div class="mb-3">
            <label for="deskripsi_promo" class="form-label">Deskripsi Promo</label>
            <textarea class="form-control" id="deskripsi_promo" name="deskripsi_promo" rows="3"><?= set_value('deskripsi_promo', $promo->deskripsi_promo) ?></textarea>
        </div>

        <div class="mb-3">
            <label for="harga_paket" class="form-label">Harga Paket</label>
            <input type="number" class="form-control" id="harga_paket" name="harga_paket" value="<?= set_value('harga_paket', $promo->harga_paket) ?>" required>
        </div>

        <div class="mb-3">
            <label for="gambar_promo" class="form-label">Gambar Promo Saat Ini</label>
            <?php if ($promo->gambar_promo): ?>
                <div class="mb-2">
                    <img src="<?= base_url('assets/uploads/' . $promo->gambar_promo) ?>" alt="<?= htmlspecialchars($promo->nama_promo) ?>" width="150">
                </div>
            <?php else: ?>
                <p>Tidak ada gambar saat ini.</p>
            <?php endif; ?>
            <input type="file" class="form-control" id="gambar_promo" name="gambar_promo">
            <small class="form-text text-muted">Biarkan kosong jika tidak ingin mengubah gambar. Ukuran maksimal 2MB. Format: JPG, PNG, JPEG, GIF, WEBP.</small>
        </div>

        <div class="mb-3">
            <label for="tanggal_mulai" class="form-label">Tanggal Mulai</label>
            <input type="date" class="form-control" id="tanggal_mulai" name="tanggal_mulai" value="<?= set_value('tanggal_mulai', $promo->tanggal_mulai) ?>" required>
        </div>

        <div class="mb-3">
            <label for="tanggal_berakhir" class="form-label">Tanggal Berakhir</label>
            <input type="date" class="form-control" id="tanggal_berakhir" name="tanggal_berakhir" value="<?= set_value('tanggal_berakhir', $promo->tanggal_berakhir) ?>" required>
        </div>

        <div class="mb-3 form-check">
            <input type="checkbox" class="form-check-input" id="status" name="status" value="active" <?= set_checkbox('status', 'active', ($promo->status == 'active')) ?>>
            <label class="form-check-label" for="status">Aktif</label>
        </div>

        <h4 class="mt-4">Item Menu dalam Promo</h4>
            ...
            <?php if (!empty($promo_items)): ?>
                <?php foreach ($promo_items as $item): ?>
                    <div class="row mb-2 menu-item-row">
                        ...
                        <div class="col-md-4">
                            <input type="number" class="form-control quantity-input" name="quantities[]" value="<?= set_value('quantities[]', $item->quantity) ?>" min="1" required>
                        </div>
                        <div class="col-md-2">
